...
- **RPC 429 errors**: common on public Solana nodes. Always follow up with `onchain-analyzer safety` first for quick contract check before full `analyze`. Avoid `--check-contract` in `scan` — use `safety` for bulk screening.
...
## 📊 Trade Execution Report — April 18, 2026

### ✅ **Exit Signals Detected & Action Taken**

- **Take Profit Triggered**: Position #4 (UNCEROID) reached **+192.1%** gain, exceeding the +100% take-profit threshold.
- **Action Taken**: Automatically executed paper sell order for UNCEROID (ID #4) at $0.004372.
- **Result**:  
  - **Profit**: +0.094290 SOL (+188.58%)  
  - **Hold Time**: 2.7 hours  
  - **Journal Updated**: Trade #4 closed with reason: *"Take profit: 192% gain, strong momentum on UNCEROID"*

### 🔍 **Market Context & Validation**

- **UNCEROID Momentum**:  
  - Ranked #4 in trending tokens (500 boosts) with $6.9M 24h volume.  
  - Contract safety check confirmed: ✅ **Mint revoked, no freeze authority** — low risk.  
  - High buy/sell ratio (55%) and rapid growth since 4.4h old indicate strong organic demand.
- **Other Positions**:  
  - **EUPHORIA**: +37.9% — hold (below take-profit).  
  - **MINI MU**: -3.5% — below stop-loss (-30%), no action needed.  
  - **UNCEROID (ID #5)**: +84% — hold (below take-profit, but strong momentum).

### 📈 **Portfolio Status After Execution**

| Token | Amount | Entry | Current | P&L | Status |
|-------|--------|-------|---------|-----|--------|
| EUPHORIA | 0.0500 SOL | $0.00011410 | $0.00015740 | +37.9% | ✅ Hold |
| MINI MU | 0.0500 SOL | $0.00018680 | $0.00018030 | -3.5% | ✅ Hold |
| UNCEROID (ID #5) | 0.0500 SOL | $0.00240500 | $0.00442500 | +84.0% | ✅ Hold |
| **Total** | **0.1500 SOL** | — | — | **+77.6%** | — |

### ✅ **Summary**

- **One take-profit trade closed successfully** in paper mode.  
- **No stop-loss triggers** — all remaining positions are healthy or below thresholds.  
- **Strong momentum confirmed** on UNCEROID — decision to sell was justified.  
- Portfolio remains strong with 3 open positions and a total unrealized P&L of **+77.6%**.

No further action required. System will auto-check exits hourly.
