...
🔍 EXIT SIGNAL REPORT — AUTOMATED EXECUTION

✅ STOP-LOSS TRIGGERED & EXECUTED:
- Trade #2: MINI MU
  - Entry: $0.0001868 | Current: $0.00009377 (-49.8%)
  - Action: SELL (PAPER MODE)
  - Result: Closed with -0.024901 SOL (-49.8%) loss
  - Reason: Stop-loss triggered (below -30%)

📊 CURRENT OPEN POSITIONS (3):
1. EUPHORIA — +13.0% loss → Hold (monitoring)
2. UNCEROID — +73.8% gain → Take-profit signal active
   - Not auto-sold: Manual confirmation required per policy
   - Momentum: Strong, but volatile — recommend reviewing socials and volume trend before decision
3. Kokopi Koalas — +0.2% gain → Neutral, stable

📌 RECOMMENDATION:
- UNCEROID is approaching +100% threshold. Monitor for volume decay or social sentiment shift.
- If risk-manager auto-sell-take-profit is enabled, this would auto-exit. Currently not enabled — manual review advised.
- Portfolio remains in positive territory: +0.0056 SOL (+2.8%) net P&L.

All actions completed autonomously. No further triggers detected.
