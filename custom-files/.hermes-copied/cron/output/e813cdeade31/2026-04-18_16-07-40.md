...
I see the issue - the executor runs `analyzer.py safety` which only returns the contract score (45/45), not the full analysis score (63/100). The risk manager then blocks it because 45 < 60 minimum. This is a known limitation - the safety command is meant for quick checks but doesn't give the full picture. Let me check if there's a way to pass the full score directly or if we should just report findings. For now, let me provide a comprehensive summary:

---

## 📊 Solana Token Scan Report

### Trending Tokens Analyzed (Top 3 with Liquidity > $10k)

| Token | Safety Score | Liquidity | Age | Status |
|-------|-------------|-----------|-----|--------|
| **MINI MU** | 63/100 ⚠️ | $33.7K | 9.8h | Contract safe, flags: low liquidity, new pair |
| **Dumb Asteroid** | 63/100 ⚠️ | $13.7K | 7.5h | Contract safe, flags: low liquidity, new pair |
| **EUPHORIA** | 68/100 ⚠️ | $29.8K | 21.4h | Contract safe, has website+socials |
...
- ⚠️ Safety scores 63-68/100 (CAUTION range, not SAFE)
- ⚠️ All flagged: LOW_LIQUIDITY, NEW_PAIR, CANNOT_READ_HOLDERS

### Current Portfolio Status
- **Open Position**: EUPHORIA (Trade #1)
  - Entry: 0.05 SOL @ $0.00011410
  - Current: ~$0.0001277 (+12% unrealized)
  - Age: 21.4h (approaching established trend phase)

### Pipeline Issue Identified
The trade executor's safety check uses `analyzer.py safety` which returns only the contract score (45/45), not the full analysis score (63-68/100). This causes risk-manager to block trades that should pass (45 < 60 minimum). 

**Recommendation**: The executor should either:
1. Use `analyzer.py analyze` output for full score, OR
2. Risk-manager should accept the full safety score from analyze, not just contract score

### No New Trades Executed
- EUPHORIA: Already held (cannot double per risk rules)
- MINI MU / Dumb Asteroid: Blocked by safety score mismatch (contract 45 vs full 63)

**Summary**: Found 3 candidates meeting criteria (score >= 60, liq > $10k). All are CAUTION-range tokens with safe contracts but early-stage risk flags. Existing EUPHORIA position is up ~12%. Pipeline needs fix to properly use full safety scores.
