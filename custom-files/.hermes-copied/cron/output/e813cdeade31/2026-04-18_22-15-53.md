...
⚠️ **NOTE**: If `CANNOT_READ_HOLDERS` flag appears, the token may have >10K holders or use a proxy. Safety score still valid if mint/freeze revoked and liquidity >$10k. Prioritize contract safety over holder data when incomplete.
...
Found one candidate: UNCEROID (BiBMG42BrmvJE5gY5UCXLURroGR6tBC1gfcYbZuEpump) with contract safety score 45/45 and $210K liquidity — passes on-chain safety. However, risk-manager blocked paper buy due to: (1) safety score 45 < min 60 requirement, and (2) existing open position in UNCEROID (trade #5). No new positions allowed. Top trending tokens either lack sufficient liquidity or violate portfolio uniqueness rules. [SILENT]
